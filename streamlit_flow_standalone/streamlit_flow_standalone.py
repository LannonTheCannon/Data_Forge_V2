...
with col1:
    if st.button("Add node"):
        new_node = StreamlitFlowNode(str(f"st-flow-node_{uuid4()}"), (0, 0),
                                     {'content': f'Node {len(st.session_state.curr_state.nodes) + 1}'}, 'default',
                                     'right', 'left')
...
with col2:
    if st.button("Delete Random Node"):
        if len(st.session_state.curr_state.nodes) > 0:
            node_to_delete = secrets.choice(st.session_state.curr_state.nodes)
            st.session_state.curr_state.nodes = [node for node in st.session_state.curr_state.nodes if
                                                 node.id != node_to_delete.id]
            st.session_state.curr_state.edges = [edge for edge in st.session_state.curr_state.edges if
                                                 edge.source != node_to_delete.id and edge.target != node_to_delete.id]
            st.rerun()
...
with col3:
    if st.button("Add Random Edge"):
        if len(st.session_state.curr_state.nodes) > 1:
            ...
            source_candidates = [streamlit_node for streamlit_node in st.session_state.curr_state.nodes if
                                 streamlit_node.type in ['input', 'default']]
            target_candidates = [streamlit_node for streamlit_node in st.session_state.curr_state.nodes if
                                 streamlit_node.type in ['default', 'output']]
            source = secrets.choice(source_candidates)
            target = secrets.choice(target_candidates)
...
with col4:
    if st.button("Delete Random Edge"):
        if len(st.session_state.curr_state.edges) > 0:
            edge_to_delete = secrets.choice(st.session_state.curr_state.edges)
            st.session_state.curr_state.edges = [edge for edge in st.session_state.curr_state.edges if
                                                 edge.id != edge_to_delete.id]
            st.rerun()

with col5:
    if st.button("Random Flow"):
        nodes = [StreamlitFlowNode(str(f"st-flow-node_{uuid4()}"), (0, 0), {'content': f'Node {i}'}, 'default', 'right',
                                   'left') for i in range(5)]
        edges = []
        for _ in range(5):
            source = secrets.choice(nodes)
            target = secrets.choice(nodes)
